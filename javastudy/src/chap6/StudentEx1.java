 ...
 *  - 필수 : 파일의 이름은 public class 클래스명 과 같다. -> StudentEx1.java
 *  - 하나의 파일 내에 하나의 public class만 가능함. 
 * Java Application
 *  - main함수가 프로그램의 시작(main함수가 프로그램의 시작점이므로 없으면 자바어플리케이션이 아님)
 *  - main함수를 포함하고 있는 클래스를 구동클래스라고 함
...
public class StudentEx1 {
	...
	public static void main(String[] args) {
		Student st1 = new Student(); //객체화/인스턴스화 : 객체를 만든거야st1이라는 st1자체는 참조변수
		/*
		 * new 예약어 기능
		 * 1. 힙영역에 메모리 할당 => 객체 생성
		 * 2. 멤버필드값을 기본값으로 초기화
		...
		st2=st1; //하나의 객체를 두개의 참조변수가 참조하게 됨
		st1.name="이몽룡"; //st1만 바꿨지만 같은 객체를 참조하기 때문에 둘 다 바뀜
		//참조변수와 객체 관계 이해하기***
