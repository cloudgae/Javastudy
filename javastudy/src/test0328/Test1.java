...
//  비정규-직원 상속
 =====이 3개의 클래스를 만들기=====
   ...
   정규직원은 급여로 연봉 / 12 로 
   비정규직원은 기본입금
   모든 클래스에 getPay() 메서드를 구현하기 //직원클래스에 있지만 정규,비정규를 알아야 작동가능->오버라이딩
...
//	정규직:이름=김정규, 주소=서울시 용산구,부서=총무부,직원번호=1,직급=과장,연봉=5000
	public String toString() {
		return type+": 이름="+name+", 주소="+address+", 부서="+dept+", 직원번호="+empNo+", 직급="+
				position+", 연봉="+salary;
	}
}
class InformalEmployee extends Employee{
	...
	//비정규직:이름=이비정, 주소=서울시 구로구,부서=영업부,계약만료일=Sun Sep 01 15:58:50 KST 2024,기본입금=1000
	public String toString() {
		return type+": 이름="+name+", 주소="+address+", 부서="+dept+", 계약만료일="+expireDate+", 기본임금="+
				primaryPay;
	}
}
public class Test1 {
