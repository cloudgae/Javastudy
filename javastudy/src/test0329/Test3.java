...
class Buyer{
	...
	int fruitp, drinkp, snackp;
	
	void buy(Food food) {
		if(money < food.price) {
			System.out.println(food+"구매시 잔액부족");
//			return; //메서드 실행 중간에 종료
		}
		//물건값 이상의 소유금액인 경우- return; 썻ㅆ으면 else없이 작성 
		else {
			this.money -= food.price; //지역변수로 선언한 적 없기 때문에 this.는 생략해도 됨
			point += food.point;
			...
			if(food instanceof Drink) {
				System.out.println(((Drink)food).ml+"ml 상품 구매");
				drinkp += food.price;
			}
	...
	void summary() {
		...
		System.out.println();
		//과일 구매금액 : 2000, 과일 구매목록 : 사과,복숭아,
		System.out.print("과일 구매금액 : " + fruitp + ", 과일 구매목록 : ");
		for(int i=0;i<cnt;i++) {
		...
		System.out.println();
		System.out.print("음료 구매금액 : " + drinkp + ", 음료 구매목록 : ");
		for(int i=0;i<cnt;i++) {
		...
		System.out.println();
		System.out.print("과자 구매금액 : " + snackp + ", 과자 구매목록 : ");
		for(int i=0;i<cnt;i++) {
